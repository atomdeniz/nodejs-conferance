...
var configuration = {
  "iceServers": [{
    ...
    "url": "stun:37.187.3.59:3478?transport=udp"
  },{
    "url": "stun:37.187.3.59:3478?transport=tcp"
  } ]
};
...
function leave(socketId) {
  ...
  if (typeof (windowEvents) === "object") {
    windowEvents.onUserLeft(socketId, friend.name);
  }
...
function loadLocalStream(muted) {
  navigator.getUserMedia({
    ...
    localStream = stream;
    if (typeof (windowEvents) === "object") {
      windowEvents.showMyVideo(localStream, muted);
    } else {
